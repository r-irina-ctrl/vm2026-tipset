...
async function loadAll() {
  ...
  try {
    ...
    state.message = "Kunde inte läsa databasen. Appen använder lokal lagring tills anslutningen fungerar.";
  }
  state.loading = false;
}
  ...
  app.innerHTML = `
    <div class="page">
      ...
      <section class="main home-main">
        ...
        <p class="config-note">Dela samma länk med deltagarna. Utan moln-tabell sparas allt lokalt på enheten.</p>
      </section>
    </div>
  `;
...
async function readStore() {
  ...
  try {
    return await readJsonBin(config);
  } catch {
...
async function getBinId(config) {
  ...
  const created = await fetch("https://api.jsonbin.io/v3/b", {
    method: "POST",
